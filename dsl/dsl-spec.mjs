...
export const MODIFIERS = [
  ...
  { name: "glyph",    signature: "glyph KIND [rotate=F] [size=F] [length=N] [stride=N]", doc: "Optional sibling clause inside a `view` block. Renders a per-cell shape (one of: arrow / dot / ring / square / plus) overlaid on the colored sphere. `rotate=FIELD` (vec2) orients the glyph by atan2(field.y, field.x). `size=FIELD` (scalar) scales the glyph by the field's magnitude. `length=N` is the base size (default 0.5, units of cell-radius). `stride=N` subsamples cells (default 1)." },
