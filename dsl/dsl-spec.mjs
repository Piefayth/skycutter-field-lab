...
export const MATH_FUNCTIONS = [
  ...
  },
  {
    ...
    arity: [1],
    argTypes: SCALAR1,    returnType: "f32",
    wgsl: passthrough("exp2"),
    js: (args) => Math.pow(2, args[0]),
    importNamespace: "core",
    signature: "exp2(x)",
    doc: "2^x. Cheap exponential when you want base-2 (binary doubling, octave-spaced kernels, decibel-style amplitudes).",
  },
  {
    name: "log",
    target: "c.log",
    arity: [1],
    argTypes: SCALAR1,    returnType: "f32",
    wgsl: passthrough("log"),
    js: (args) => Math.log(args[0]),
    importNamespace: "core",
    signature: "log(x)",
    doc: "Natural log (base e). Argument must be > 0; behavior at 0 / negatives is platform-dependent — clamp upstream if your dynamics can produce non-positive values.",
  },
  {
    name: "log2",
    target: "c.log2",
    arity: [1],
    argTypes: SCALAR1,    returnType: "f32",
    wgsl: passthrough("log2"),
    js: (args) => Math.log2(args[0]),
    importNamespace: "core",
    signature: "log2(x)",
    doc: "Base-2 log. Same domain caveat as log.",
  },
  {
    name: "tanh",
    target: "c.tanh",
    arity: [1],
    argTypes: SCALAR1,    returnType: "f32",
    wgsl: passthrough("tanh"),
    js: (args) => Math.tanh(args[0]),
    importNamespace: "core",
    signature: "tanh(x)",
    doc: "Hyperbolic tangent — saturating S-curve in [-1, 1]. Use for sigmoid kinetics (saturating drive, rate-limited activation), neural-style activations, and any reaction-diffusion term that should asymptote rather than blow up.",
    example: "let drive = tanh(activator * gain)",
  },
  {
    name: "mod",
    target: "Math.mod",
    arity: [2],
    argTypes: SCALAR2,    returnType: "f32",
    // WGSL's `%` for floats is truncated-division remainder; this gives
    // negative results for negative dividends. We want the floored
    // (mathematical) modulo so `mod(-0.1, 1) == 0.9` — useful for
    // wrapping coordinates / phases. JS `%` has the same truncated
    // semantics, hence the `((a % b) + b) % b` idiom on the JS side.
    wgsl: (args) => `(${args[0]} - floor(${args[0]} / ${args[1]}) * ${args[1]})`,
    js: (args) => ((args[0] % args[1]) + args[1]) % args[1],
    importNamespace: "core",
    signature: "mod(a, b)",
    doc: "Floored modulo — result has the sign of b. Always returns a value in [0, b) when b > 0, regardless of whether a is negative. Different from `%` which is truncated and produces negative remainders for negative dividends. Use for wrapping angles into [0, TAU), folding coordinates onto a periodic domain, etc.",
    example: "let wrapped = mod(theta, TAU)",
  },
  {
    name: "sqrt",
    target: "c.sqrt",
...
export const MODIFIERS = [
  ...
  { name: "particles", signature: "particles advect=VEC2_FIELD [count=N] [length=N] [speed=N] [fade=N] [size=N] [color [R,G,B]]", doc: "Optional sibling clause inside a `view` block. Renders deterministic tracer particles advected through a vec2 tangent field, with fading trails. Render-only: it never writes simulation state." },
  { name: "advect",   signature: "particles advect=VEC2_FIELD",                  doc: "Required named arg on `particles`. Vec2 field sampled as the visual tracer velocity in the cell's east/north tangent frame." },
  ...
  { name: "size",     signature: "glyph ... size=SCALAR_FIELD / particles ... size=N", doc: "Optional named arg on `glyph` and `particles`. Glyph: scalar field that multiplies glyph size per cell. Particles: visual tracer size on the globe surface, default 6." },
  { name: "length",   signature: "glyph ... length=N / particles ... length=N",   doc: "Optional named arg on `glyph` and `particles`. Glyph: base size in units of cell-radius. Particles: number of stored trail points. Defaults: glyph 0.5, particles 16." },
  ...
  { name: "count",    signature: "particles ... count=N",                         doc: "Optional named arg on `particles`. Number of visual tracers. Default 2400; validator caps at 20000." },
  { name: "speed",    signature: "particles ... speed=N",                         doc: "Optional named arg on `particles`. Visual tracer speed multiplier. It does not affect simulation state." },
  { name: "fade",     signature: "particles ... fade=N",                          doc: "Optional named arg on `particles`. Trail intensity falloff in [0, 1]." },
];
